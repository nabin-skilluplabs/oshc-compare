...
const testDbPath = resolve("prisma/test.db");
...
before(async () => {
  ...
  quoteService = await import("../src/services/quote-service.js");
...
describe("quote service", () => {
  it("calculates cover types from household composition", () => {
    assert.equal(quoteService.calculateCoverType(1, 0), "single");
    assert.equal(quoteService.calculateCoverType(2, 0), "couple");
    assert.equal(quoteService.calculateCoverType(1, 1), "single_parent_family");
    assert.equal(quoteService.calculateCoverType(2, 2), "family");
  });

  it("calculates policy duration in days", () => {
    const start = new Date("2026-07-01T00:00:00.000Z");
    const end = new Date("2027-07-01T00:00:00.000Z");
    assert.equal(quoteService.calculateDurationDays(start, end), 365);
  });
});

describe("public quote and results API", () => {
...
describe("checkout and payment API", () => {
